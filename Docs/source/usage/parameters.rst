...
* ``warpx.end_fine_patch_step`` (`integer`, default: ``-1``)
    When using mesh refinement, this can be used to remove the finest level at a given timestep.
        Note: As soon as the fine patch is removed, ``max_step`` is modified so that the final step corresponds to the physical time calculated with the initial timestep duration. Therefore, users are advised to use ``stop_time`` instead.
